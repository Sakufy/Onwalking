...
import { GoldQuote, Monologue, Summary, Dialogue } from './extensions'
...
export default function DualPaneEditor({
  ...
  const handleAddDialogueItem = () => {
    editor?.chain().focus().insertContent('<p><strong>角色：</strong>对话内容</p>').run()
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="min-h-screen bg-gray-50">
      <div className="flex items-center justify-between mb-6 px-6 py-4 bg-white border-b border-gray-200">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gray-50">
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={() => editor.chain().focus().toggleNode('summary', 'paragraph').run()}
                className={`p-2 rounded hover:bg-gray-200 transition-colors ${
                  editor.isActive('summary') ? 'bg-gray-200' : ''
                }`}
                title="摘要段"
              >
              ...
              <button
                type="button"
                onClick={() => editor.chain().focus().toggleNode('goldQuote', 'paragraph').run()}
                className={`p-2 rounded hover:bg-gray-200 transition-colors ${
                  editor.isActive('goldQuote') ? 'bg-gray-200' : ''
                }`}
                title="金句"
              >
                <span className="text-xs font-medium">金句</span>
              </button>
              <button
                type="button"
                onClick={() => editor.chain().focus().toggleNode('monologue', 'paragraph').run()}
                className={`p-2 rounded hover:bg-gray-200 transition-colors ${
                  editor.isActive('monologue') ? 'bg-gray-200' : ''
                }`}
                title="独白"
              >
                <span className="text-xs font-medium">独白</span>
              </button>
              <button
                type="button"
                onClick={() => editor.chain().focus().toggleNode('dialogue', 'paragraph').run()}
                className={`p-2 rounded hover:bg-gray-200 transition-colors ${
                  editor.isActive('dialogue') ? 'bg-gray-200' : ''
                }`}
                title="对话"
              >
                <span className="text-xs font-medium">对话</span>
              </button>
              {editor.isActive('dialogue') && (
                <button
                  type="button"
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
                  activeTab === 'edit' ? 'bg-gray-200' : 'hover:bg-gray-100'
                }`}
              >
                编辑
              </button>
              <button
                ...
                className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
                  activeTab === 'split' ? 'bg-gray-200' : 'hover:bg-gray-100'
                }`}
              >
                分屏
              </button>
          ...
          <div className="flex">
            {(activeTab === 'edit' || activeTab === 'split') && (
              <div className={`border-r border-gray-200 ${activeTab === 'split' ? 'w-1/2' : 'w-full'}`}>
                <div className="p-6 min-h-[600px]">
                  <EditorContent editor={editor} className="min-h-[500px] focus:outline-none prose" />
                </div>
              </div>
            )}
            {(activeTab === 'preview' || activeTab === 'split') && (
              <div className={`bg-gray-50 ${activeTab === 'split' ? 'w-1/2' : 'w-full'}`}>
                <div className="p-6 min-h-[600px]">
                  <div className="max-w-2xl mx-auto">
                    <div className="bg-white rounded-lg shadow-sm p-8">
                      <h1 className="text-2xl font-bold text-gray-900 mb-2">{title || '文章标题'}</h1>
                      <div className="text-sm text-gray-500 mb-6">
                        {category && <span className="inline-block px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs mr-2">{category}</span>}
                        <span>发布时间</span>
                      </div>
                      <ArticleRenderer content={editor.getHTML()} />
                    </div>
                  </div>
                </div>
              </div>
